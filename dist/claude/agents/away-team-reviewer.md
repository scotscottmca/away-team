...
tools: Read, Grep, Glob, Bash, Edit, Write, NotebookEdit, TodoWrite, mcp__ado__*, mcp__azure-devops__*, mcp__github__*, mcp__github-mcp-server__*, ToolSearch
model: sonnet
effort: medium
maxTurns: 40
---
...
1. **Preflight.** Prefer the GitHub MCP server if `mcp__github__*` (Claude Code) or `github/*` (Copilot) tools are on your allowlist. On Claude Code they can be deferred — off your tool list, and a direct call fails, until `ToolSearch` loads them (`select:mcp__github__pull_request_read`) — so load them before reading anything into a tool list that does not show them. Only then fall back to an authenticated `gh` (`gh auth status`; a hosted or web session ships none). Neither → Blocked (stage: preflight). Check out the PR's head branch if it is not already checked out; a dirty working tree → Blocked (stage: preflight), never stash or discard someone's work.
