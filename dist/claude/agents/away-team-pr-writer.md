...
1. **Preflight.** Check `git remote -v` and which GitHub path you have: prefer the GitHub MCP server if `mcp__github__*` (Claude Code) or `github/*` (Copilot) tools are on your allowlist. On Claude Code they can be deferred — off your tool list, and a direct call fails, until `ToolSearch` loads them (`select:mcp__github__create_pull_request`) — so load them before reading anything into a tool list that does not show them. Only then fall back to `gh auth status`, which a hosted or web session fails because it ships no `gh` binary. Neither an MCP GitHub server nor an authenticated `gh`, or no remote → Blocked (stage: preflight). Never add, remove or change a remote to get past this; the user decides where code goes.
