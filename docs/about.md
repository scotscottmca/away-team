...
Agents declare a tier, not a model, so the same file runs on whatever each platform has.
...
Copilot and Claude Code want different tool names and different model identifiers in the frontmatter, and nothing else differs. The installer rewrites those two lines at install time. The same code also renders a `dist/` folder per platform, committed to the repo, so both tools' plugin marketplaces can install it straight from GitHub.

## Deployment

The package publishes to npm as `@scotscottmca/away-team` from a GitHub Actions workflow. A push to `main` that touches an agent, a skill or the installer bumps the patch version, rebuilds `dist/`, tags and publishes. Documentation-only changes don't release. A user installs with one `npx` command on Windows, macOS or Linux, or through the Copilot and Claude Code plugin marketplaces.

[Visit →](https://github.com/scotscottmca/away-team)
